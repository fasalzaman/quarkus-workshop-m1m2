...
Back on the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-codeready[Topology View^], make sure all the Kafka and Zookeeper pods are up and running (with dark blue circles):

image::M2-kafka.PNG[topiccreate,800]
...
----
mvn -q quarkus:add-extension -Dextensions="messaging-kafka" -Duser.home=/home/jboss -f $CHE_PROJECTS_ROOT/quarkus-workshop-m1m2-labs
----
