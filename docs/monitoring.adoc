...
On the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-codeready[Topology View for your project^], you'll see prometheus spinning up.
...
image::M2-promath.PNG[Prometheus, 700]
...
[source,sh,role="copypaste"]
----
mvn -q quarkus:add-extension -Dextensions="smallrye-metrics" -Duser.home=/home/jboss -f $CHE_PROJECTS_ROOT/quarkus-workshop-m1m2-labs
----
...
Once the build completes, ensure the app completes its redeployment with this command (or watch the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-codeready[Topology View for your project^])
...
You'll need to trigger the methods that we've instrumented, so http://people-{{USER_ID}}-codeready.{{ROUTE_SUBDOMAIN}}/names.html[reopen the name cloud^], which will start producing names (and generating metrics):

image::names.png[names, 400]

Within about 15-30 seconds, Prometheus should start scraping the metrics. Once again, access the http://prometheus-{{USER_ID}}-codeready.{{ROUTE_SUBDOMAIN}}[Prometheus UI^]. Start typing in the query box to look for 'acme':
...
Follow the same process as before: On the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-codeready[Topology View^], click on `+Add`, and choose "Container Image", and fill in the fields:
...
On the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-codeready[Topology View for your project^], you'll see Grafana spinning up. Once it completes, click on the arrow to access the Grafana UI:

image::M2-Grafana.PNG[Prometheus, 700]
...
* *URL*: `http://prometheus.{{USER_ID}}-codeready:9090`
...
Back on the http://grafana-{{USER_ID}}-codeready.{{ROUTE_SUBDOMAIN}}[Grafana Home^], select *New Dashboard* to create a new _Dashboard_ to review the metrics.
