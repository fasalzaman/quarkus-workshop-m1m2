...
First, on the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-codeready[Topology View^], click **+Add** and, then _From Catalog_:
...
----
mvn -q quarkus:add-extension -Dextensions="smallrye-opentracing, rest-client" -Duser.home=/home/jboss -f $CHE_PROJECTS_ROOT/quarkus-workshop-m1m2-labs
----
...
You'll need to trigger some HTTP endpoints to generate traces. Access the http://people-{{USER_ID}}-codeready.{{ROUTE_SUBDOMAIN}}/datatable.html[graphical person browser powered by the DataTables library we created earlier^].
...
Open the https://jaeger-{{USER_ID}}-codeready.{{ROUTE_SUBDOMAIN}}[Jaeger Query UI^]. By default Jaeger uses the same login details as OpenShift, so click the **Login with OpenShift** button, enter your credentials:
...
Reload the https://jaeger-{{USER_ID}}-codeready.{{ROUTE_SUBDOMAIN}}[Jaeger Query UI^], and click **Find Traces**. The new trace should appear the top with multiple spans. Click on it to display details:
